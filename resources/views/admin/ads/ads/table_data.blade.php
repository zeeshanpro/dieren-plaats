           ...
           <?php //print_r($result); ?>
                    @foreach($result as $row)
                      ...
                      <td title="<?php if( $row->status == 0 ) echo 'Paused Ad';?>">
                        ...
                        <a href="{{url('/admin/adsdetail/'.$row->id)}}" ><img src="{{ url('storage/app/public/uploads/ads/thumb/'.$row->adImages[0]->filename) }}" class="imageStyle <?php if( $row->status == 0 ) echo 'border border-danger'?>" alt="None" style="width: 50px;height:50px;">
                        </a>
                        @else
                        No Image
                        @endif
